...
public client class Client {
    ...
    remote function checkRedisCacheNameAvailability(string cacheName) returns @tainted StatusCode|error {
        ...
        json checkPayload = {"type": "Microsoft.Cache/Redis", "name": cacheName };
        ...
        if (response is http:Response) {
            ...
            if(statusCode == "401"){
                ...
                io:println("DNS name for the cache instance not available or Temporary not available due to recently deleted");
                return jsonToStatusCode(statusCode);
            }
        } else {
    ...
    remote function createRedisCache(string redisCacheName, string resourceGroupName, string location, CreateCacheProperty properties, string[]? zones = ()) returns @tainted RedisCacheInstance|error {

        if (location == EMPTY_STRING || properties.sku.name == EMPTY_STRING || properties.sku.family == EMPTY_STRING || properties.sku.capacity.toString() == EMPTY_STRING) {
            return createError("Required values not provided");
        }
        
        var path = "/resourceGroups/"+resourceGroupName+"/providers/Microsoft.Cache/redis/"+redisCacheName+"?api-version="+config:getAsString("API_VERSION");
        http:Request request = new;
        json createCacheJsonPayload = {"location": location, "properties": { "sku": { "name": properties.sku.name, "family": properties.sku.family, "capacity": properties.sku.capacity}, "enableNonSslPort":true}};
...
//that your Redis Database (RDB) file or files are uploaded into page or block blobs in Azure storage, in the same region and subscription as your Azure Cache for Redis instance.
    ...
    # + return - If successful, returns StatusCode. Else returns error. 
    remote function exportRedisCache(string redisCacheName, string resourceGroupName, string prefix, string blobContainerUrl, string sasKeyParameters, string? format = ()) returns @tainted StatusCode|error {

        if (redisCacheName == EMPTY_STRING || resourceGroupName == EMPTY_STRING || prefix == EMPTY_STRING || blobContainerUrl == EMPTY_STRING) {
            return createError("Required values not provided");
        }

        var path = "/resourceGroups/"+resourceGroupName+"/providers/Microsoft.Cache/redis/"+redisCacheName+"/export?api-version="+config:getAsString("API_VERSION");
        http:Request request = new;
        json exportCacheJsonPayload = {"format": format, "prefix": prefix, "container": blobContainerUrl + sasKeyParameters};
...
//that your Redis Database (RDB) file or files are uploaded into page or block blobs in Azure storage, in the same region and subscription as your Azure Cache for Redis instance.
    ...
    # + return - If successful, returns StatusCode. Else returns error. 
    remote function importRedisCache(string redisCacheName, string resourceGroupName, string files, string? format = ()) returns @tainted StatusCode|error {

        if (redisCacheName == EMPTY_STRING || resourceGroupName == EMPTY_STRING || files == EMPTY_STRING) {
            return createError("Required values not provided");
        }

        var path = "/resourceGroups/"+resourceGroupName+"/providers/Microsoft.Cache/redis/"+redisCacheName+"/import?api-version="+config:getAsString("API_VERSION");
        http:Request request = new;
    ...
    # + resourceGroupName - Resource Group Name where Redis Cache found. 
    # + return - If successful, returns RedisCacheInstance. Else returns error. 
    remote function getRedisCache(string redisCacheName, string resourceGroupName) returns @tainted RedisCacheInstance|error {
    ...
    # + return - If successful, returns StatusCode. Else returns error. 
    remote function forceRebootRedisCache(string redisCacheName, string resourceGroupName, int? shardId = (), string? rebootType = (), int[]? ports = ()) returns @tainted StatusCode|error {

        if (redisCacheName == EMPTY_STRING || resourceGroupName == EMPTY_STRING) {
            return createError("Required values not provided");
        }

        var path = "/resourceGroups/"+resourceGroupName+"/providers/Microsoft.Cache/redis/"+redisCacheName+"/forceReboot?api-version="+config:getAsString("API_VERSION");
        http:Request request = new;
        json rebootCacheJsonPayload = {"shardId": shardId, "rebootType": rebootType, "ports": ports};
    ...
    remote function listByResourceGroup(string resourceGroupName) returns @tainted StatusCode|error {
        ...
        var path = "/resourceGroups/"+resourceGroupName+"/providers/Microsoft.Cache/redis?api-version="+config:getAsString("API_VERSION");
        http:Request request = new;
        var response = self.AzureRedisClient->get(<@untainted>path, request);
        if (response is http:Response) {
            string statusCode = response.statusCode.toString();
    ...
    remote function listKeys(string redisCacheName, string resourceGroupName) returns @tainted StatusCode|error {
        ...
        var path = "/resourceGroups/"+resourceGroupName+"/providers/Microsoft.Cache/redis/"+redisCacheName+"/listKeys?api-version="+config:getAsString("API_VERSION");
        http:Request request = new;
        var response = self.AzureRedisClient->post(<@untainted>path, request);
        if (response is http:Response) {
            string statusCode = response.statusCode.toString();
    ...
    # + return - Return Value Description
    remote function listUpgradeNotifications(string redisCacheName, string resourceGroupName, float history) returns @tainted StatusCode|error {

        if (redisCacheName == EMPTY_STRING || resourceGroupName == EMPTY_STRING || history.toString() == EMPTY_STRING) {
            return createError("Required values not provided");
        }
        
        var path = "/resourceGroups/"+resourceGroupName+"/providers/Microsoft.Cache/redis/"+redisCacheName+"/listUpgradeNotifications?api-version="+config:getAsString("API_VERSION")+"&history="+history.toString();
        http:Request request = new;
        var response = self.AzureRedisClient->get(<@untainted>path, request);
        if (response is http:Response) {
            string statusCode = response.statusCode.toString();
    ...
    # + return - Return Value Description
    remote function regenerateKey(string redisCacheName, string resourceGroupName, string keyType) returns @tainted StatusCode|error {

        if (redisCacheName == EMPTY_STRING || resourceGroupName == EMPTY_STRING || keyType == EMPTY_STRING) {
            return createError("Required values not provided");
        }
        
        var path = "/resourceGroups/"+resourceGroupName+"/providers/Microsoft.Cache/redis/"+redisCacheName+"/regenerateKey?api-version="+config:getAsString("API_VERSION");
        http:Request request = new;
    ...
    # + return - If successful, returns RedisCacheInstance. Else returns error. 
    remote function updateRedisCache(string redisCacheName, string resourceGroupName, CreateCacheProperty properties) returns @tainted json|error {

        if (properties.sku.name == EMPTY_STRING || properties.sku.family == EMPTY_STRING || properties.sku.capacity.toString() == EMPTY_STRING) {
            return createError("Required values not provided");
        }
    ...
    remote function createFirewallRule(string redisCacheName, string resourceGroupName, string ruleName, string startIP, string endIP) returns @tainted FirewallRuleResponse|error {

        if (redisCacheName == EMPTY_STRING || resourceGroupName == EMPTY_STRING || ruleName == EMPTY_STRING || startIP == EMPTY_STRING || endIP == EMPTY_STRING) {
            return createError("Required values not provided");
        }

        var path = "/resourceGroups/"+resourceGroupName+"/providers/Microsoft.Cache/redis/"+redisCacheName+"/firewallRules/"+ruleName+"?api-version="+config:getAsString("API_VERSION");
        http:Request request = new;
        json createFilewallRuleJsonPayload = {"properties": {"startIP": startIP, "endIP": endIP}};
        ...
        if (response is http:Response) {
            ...
            if(statusCode == "401"){
                ...
                if(createFirewallJsonResponse is json){
                    FirewallRuleResponse createFirewallResponse = check <@untainted>createFirewallJsonResponse.cloneWithType(FirewallRuleResponse);
    ...
    # + return - If successful, returns StatusCode. Else returns error. 
    remote function deleteFirewallRule(string redisCacheName, string resourceGroupName, string ruleName) returns @tainted StatusCode|error {
    ...
    # + ruleName - Name of Rule Name
    # + return - If successful, returns FirewallRuleResponse. Else returns error. 
    remote function getFirewallRule(string redisCacheName, string resourceGroupName, string ruleName) returns @tainted FirewallRuleResponse|error {
        ...
        if (response is http:Response) {
            ...
            if(statusCode == "401"){
                ...
                if(getFirewallJsonResponse is json){
                    FirewallRuleResponse getFirewallResponse = check <@untainted>getFirewallJsonResponse.cloneWithType(FirewallRuleResponse);
    ...
    remote function listFirewallRules(string redisCacheName, string resourceGroupName) returns @tainted FirewallRuleListResponse|error {

        if (redisCacheName == EMPTY_STRING || resourceGroupName == EMPTY_STRING) {
            return createError("Required values not provided");
        }

        var path = "/resourceGroups/"+resourceGroupName+"/providers/Microsoft.Cache/redis/"+redisCacheName+"/firewallRules?api-version="+config:getAsString("API_VERSION");
        http:Request request = new;
        var response = self.AzureRedisClient->get(<@untainted>path, request);
        if (response is http:Response) {
            string statusCode = response.statusCode.toString();
            ...
            if(statusCode == "401"){
                ...
                if(listFirewallJsonResponse is json){
                    FirewallRuleListResponse getFirewallResponseArray = check <@untainted>listFirewallJsonResponse.cloneWithType(FirewallRuleListResponse);
    ...
    remote function createLinkedServer(string redisCacheName, string resourceGroupName, string linkedServerName, string linkedRedisCacheId, string linkedRedisCacheLocation, string serverRole) returns @tainted LinkedServer|error {

        if (redisCacheName == EMPTY_STRING || resourceGroupName == EMPTY_STRING || linkedServerName == EMPTY_STRING || linkedRedisCacheId == EMPTY_STRING || linkedRedisCacheLocation == EMPTY_STRING || serverRole == EMPTY_STRING) {
            return createError("Required values not provided");
        }

        var path = "/resourceGroups/"+resourceGroupName+"/providers/Microsoft.Cache/redis/"+redisCacheName+"/linkedServers/"+linkedServerName+"?api-version="+config:getAsString("API_VERSION");
        http:Request request = new;
        json createLinkedServerJsonPayload = {
            "properties": {
            ...
            io:println(createLinkedServer);
                if(statusCode == "401"){
                    ...
                    if(createLinkedServer is json){
                        ...
                        LinkedServer createLinkedServerResponse = check <@untainted>createLinkedServer.cloneWithType(LinkedServer);
    ...
    # + linkedServerName - Name of Linked Server Name
    # + return - If successful, returns StatusCode. Else returns error. 
    remote function deleteLinkedServer(string redisCacheName, string resourceGroupName, string linkedServerName) returns @tainted StatusCode|error {
    ...
    # + linkedServerName - Name of Linked Server Name
    # + return - If successful, returns LinkedServer. Else returns error. 
    remote function getLinkedServer(string redisCacheName, string resourceGroupName, string linkedServerName) returns @tainted LinkedServer|error {
    ...
    # + resourceGroupName - Resource Group Name where Redis Cache found. 
    # + return - If successful, returns LinkedServer[]. Else returns error. 
    remote function listLinkedServers(string redisCacheName, string resourceGroupName) returns @tainted StatusCode|error {

        if (redisCacheName == EMPTY_STRING || resourceGroupName == EMPTY_STRING) {
            return createError("Required values not provided");
        }

        var path = "/resourceGroups/"+resourceGroupName+"/providers/Microsoft.Cache/redis/"+redisCacheName+"/linkedServers?api-version="+config:getAsString("API_VERSION");
        http:Request request = new;
        var response = self.AzureRedisClient->get(<@untainted>path, request);
        if (response is http:Response) {
            string statusCode = response.statusCode.toString();
    ...
    remote function createPatchSchedule(string redisCacheName, string resourceGroupName, PatchScheduleProperty? patchScheduleProperties) returns @tainted PatchShedule|error {

        if (redisCacheName == EMPTY_STRING || resourceGroupName == EMPTY_STRING) {
            return createError("Required values not provided");
        }

        var path = "/resourceGroups/"+resourceGroupName+"/providers/Microsoft.Cache/redis/"+redisCacheName+"/patchSchedules/default?api-version="+config:getAsString("API_VERSION");
        http:Request request = new;
        ...
        if (response is http:Response) {
            ...
            if(statusCode == "401"){
                ...
                if(createPatchJsonResponse is json){
                    PatchShedule createPatchResponse = check <@untainted>createPatchJsonResponse.cloneWithType(PatchShedule);
    ...
    # + resourceGroupName - Resource Group Name where Redis Cache found. 
    # + return - If successful, returns StatusCode. Else returns error. 
    remote function deletePatchSchedule(string redisCacheName, string resourceGroupName) returns @tainted StatusCode|error {
    ...
    # + resourceGroupName - Resource Group Name where Redis Cache found.
    # + return - If successful, returns PatchSchedule. Else returns error. 
    remote function getPatchSchedule(string redisCacheName, string resourceGroupName) returns @tainted PatchShedule|error {
    ...
    # + resourceGroupName - Resource Group Name where Redis Cache found.
    # + return - If successful, returns PatchSchedule[]. Else returns error. 
    remote function listPatchSchedules(string redisCacheName, string resourceGroupName) returns @tainted PatchSheduleList|error {

        if (redisCacheName == EMPTY_STRING || resourceGroupName == EMPTY_STRING) {
            return createError("Required values not provided");
        }

        var path = "/resourceGroups/"+resourceGroupName+"/providers/Microsoft.Cache/redis/"+redisCacheName+"/patchSchedules?api-version="+config:getAsString("API_VERSION");
        http:Request request = new;
        var response = self.AzureRedisClient->get(<@untainted>path, request);
        if (response is http:Response) {
            string statusCode = response.statusCode.toString();
            ...
            if(statusCode == "401"){
                ...
                if(PatchSheduleJsonResponse is json){
                    PatchSheduleList getPatchSheduleArray = check <@untainted>PatchSheduleJsonResponse.cloneWithType(PatchSheduleList);
    ...
    remote function putPrivateEndpointConnection(string redisCacheName, string resourceGroupName, string privateEndpointConnectionName) returns @tainted json|error {

        if (redisCacheName == EMPTY_STRING || resourceGroupName == EMPTY_STRING || privateEndpointConnectionName == EMPTY_STRING) {
            return createError("Required values not provided");
        }

        var path = "/resourceGroups/"+resourceGroupName+"/providers/Microsoft.Cache/redis/"+redisCacheName+"/privateEndpointConnections/"+privateEndpointConnectionName+"?api-version="+config:getAsString("API_VERSION");
        http:Request request = new;
        json putPrivateEndpointConnectionJsonPayload = {
                                            "properties": {
    ...
    # + privateEndpointConnectionName - Name of Private Endpoint Connection
    # + return - If successful, returns PrivateEndpointConnection. Else returns error. 
    remote function getPrivateEndpointConnection(string redisCacheName, string resourceGroupName, string privateEndpointConnectionName) returns @tainted json|error {
    ...
    # + resourceGroupName - Resource Group Name where Redis Cache found.
    # + return - If successful, returns PrivateEndpointConnection[]. Else returns error.
    remote function listPrivateEndpointConnection(string redisCacheName, string resourceGroupName) returns @tainted StatusCode|error {

        if (redisCacheName == EMPTY_STRING || resourceGroupName == EMPTY_STRING) {
            return createError("Required values not provided");
        }

        var path = "/resourceGroups/"+resourceGroupName+"/providers/Microsoft.Cache/redis/"+redisCacheName+"/privateEndpointConnections?api-version="+config:getAsString("API_VERSION");
        http:Request request = new;
        var response = self.AzureRedisClient->get(<@untainted>path, request);
        if (response is http:Response) {
            string statusCode = response.statusCode.toString();
    ...
    # + privateEndpointConnectionName - Name of Private Endpoint Connection
    # + return - If successful, returns StatusCode. Else returns error.
    remote function deletePrivateEndpointConnection(string redisCacheName, string resourceGroupName, string privateEndpointConnectionName) returns @tainted StatusCode|error {
    ...
    remote function getPrivateLinkResources(string redisCacheName, string resourceGroupName) returns @tainted StatusCode|error {

        var path = "/resourceGroups/"+resourceGroupName+"/providers/Microsoft.Cache/redis/"+redisCacheName+"/privateLinkResources?api-version="+config:getAsString("API_VERSION");
        http:Request request = new;
        var response = self.AzureRedisClient->get(<@untainted>path, request);
        if (response is http:Response) {
            string statusCode = response.statusCode.toString();
    ...
    remote function createRedisEnterpriseCacheDatabase(string redisEnterpriseCacheName, string resourceGroupName, string databaseName) returns @tainted StatusCode|error {

        var path = "/resourceGroups/"+resourceGroupName+"/providers/Microsoft.Cache/redisEnterprise/"+redisEnterpriseCacheName+"/databases/"+databaseName+"?api-version="+config:getAsString("ENTERPRISE_API_VERSION");
        http:Request request = new;
        json createEnterpriseCacheDBJsonPayload = {
                                                    "properties": {
    ...
    # + databaseName - Name of Database Name
    # + return - If successful, returns StatusCode. Else returns error. 
    remote function deleteRedisEnterpriseCacheDatabase(string redisEnterpriseCacheName, string resourceGroupName, string databaseName) returns @tainted StatusCode|error {
    ...
    # + sasKeyParameters - SAS key
    # + return - If successful, returns StatusCode. Else returns error. 
    remote function exportRedisEnterpriseCacheDatabase(string redisEnterpriseCacheName, string resourceGroupName, string databaseName, string blobContainerUrl, string sasKeyParameters) returns @tainted StatusCode|error {

        if (redisEnterpriseCacheName == EMPTY_STRING || resourceGroupName == EMPTY_STRING || sasKeyParameters == EMPTY_STRING || blobContainerUrl == EMPTY_STRING) {
            return createError("Required values not provided");
        }

        var path = "/resourceGroups/"+resourceGroupName+"/providers/Microsoft.Cache/redisEnterprise/"+redisEnterpriseCacheName+"/databases/"+databaseName+"/export?api-version="+config:getAsString("ENTERPRISE_API_VERSION");
        http:Request request = new;
    ...
    # + sasKeyParameters - SAS key
    # + return - If successful, returns StatusCode. Else returns error. 
    remote function importRedisEnterpriseCacheDatabase(string redisEnterpriseCacheName, string resourceGroupName, string databaseName, string blobFileUrl, string sasKeyParameters) returns @tainted StatusCode|error {

        if (redisEnterpriseCacheName == EMPTY_STRING || resourceGroupName == EMPTY_STRING) {
            return createError("Required values not provided");
        }

        var path = "/resourceGroups/"+resourceGroupName+"/providers/Microsoft.Cache/redisEnterprise/"+redisEnterpriseCacheName+"/databases/"+databaseName+"/import?api-version="+config:getAsString("ENTERPRISE_API_VERSION");
        http:Request request = new;
    ...
    # + databaseName - Name of Database Name
    # + return - If successful, returns RedisEnterpriseCacheDatabase. Else returns error. 
    remote function getRedisEnterpriseCacheDatabase(string redisEnterpriseCacheName, string resourceGroupName, string databaseName) returns @tainted StatusCode|error {
    ...
    # + resourceGroupName - Resource Group Name where Redis Cache found.
    # + return - If successful, returns RedisEnterpriseCacheDatabase[]. Else returns error. 
    remote function listRedisEnterpriseCacheDatabaseByCluster(string redisEnterpriseCacheName, string resourceGroupName) returns @tainted StatusCode|error {

        if (redisEnterpriseCacheName == EMPTY_STRING || resourceGroupName == EMPTY_STRING) {
            return createError("Required values not provided");
        }
        
        var path = "/resourceGroups/"+resourceGroupName+"/providers/Microsoft.Cache/redisEnterprise/"+redisEnterpriseCacheName+"/databases?api-version="+config:getAsString("ENTERPRISE_API_VERSION");
        http:Request request = new;
        var response = self.AzureRedisClient->get(<@untainted>path, request);
        if (response is http:Response) {
            string statusCode = response.statusCode.toString();
    ...
    remote function listRedisEnterpriseCacheDatabaseKeys(string redisEnterpriseCacheName, string resourceGroupName, string databaseName) returns @tainted StatusCode|error {

        if (redisEnterpriseCacheName == EMPTY_STRING || resourceGroupName == EMPTY_STRING) {
            return createError("Required values not provided");
        }
        
        var path = "/resourceGroups/"+resourceGroupName+"/providers/Microsoft.Cache/redisEnterprise/"+redisEnterpriseCacheName+"/databases/"+databaseName+"/listKeys?api-version="+config:getAsString("ENTERPRISE_API_VERSION");
        http:Request request = new;
        var response = self.AzureRedisClient->post(<@untainted>path, request);
        if (response is http:Response) {
            string statusCode = response.statusCode.toString();
    ...
    # + databaseName - Name of Database Name
    # + return - If successful, returns StatusCode. Else returns error. 
    remote function regenerateRedisEnterpriseCacheDatabaseKey(string redisEnterpriseCacheName, string resourceGroupName, string databaseName, string keyType) returns @tainted StatusCode|error {

        if (redisEnterpriseCacheName == EMPTY_STRING || resourceGroupName == EMPTY_STRING || keyType == EMPTY_STRING) {
            return createError("Required values not provided");
        }
        
        var path = "/resourceGroups/"+resourceGroupName+"/providers/Microsoft.Cache/redisEnterprise/"+redisEnterpriseCacheName+"/databases/"+databaseName+"/regenerateKey?api-version="+config:getAsString("ENTERPRISE_API_VERSION");
        http:Request request = new;
        json regenerateKeyJsonPayload = {"keyType": keyType};
        request.setJsonPayload(regenerateKeyJsonPayload);
        var response = self.AzureRedisClient->post(<@untainted>path, request);
    ...
    # 
    # + return - If successful, returns RedisEnterpriseCacheDatabase. Else returns error. 
    remote function updateRedisEnterpriseCacheDatabase(string redisEnterpriseCacheName, string resourceGroupName, string databaseName, string clientProtocol, string evictionPolicy, string moduleName) returns @tainted StatusCode|error {
        ...
        var path = "/resourceGroups/"+resourceGroupName+"/providers/Microsoft.Cache/redisEnterprise/"+redisEnterpriseCacheName+"/databases/"+databaseName+"?api-version="+config:getAsString("ENTERPRISE_API_VERSION");
        http:Request request = new;
        json updateCacheJsonPayload = {"properties": {"clientProtocol": clientProtocol, "evictionPolicy": evictionPolicy, "modules": [{"name": moduleName}]}};
        request.setJsonPayload(updateCacheJsonPayload);
        var response = self.AzureRedisClient->patch(<@untainted>path, request);
        if (response is http:Response) {
            string statusCode = response.statusCode.toString();
    ...
    remote function createRedisEnterprise(string redisEnterpriseClusterName, string resourceGroupName, string location, string skuName, int skuCapacity) returns @tainted StatusCode|error {

        var path = "/resourceGroups/"+resourceGroupName+"/providers/Microsoft.Cache/redisEnterprise/"+redisEnterpriseClusterName+"?api-version="+config:getAsString("ENTERPRISE_API_VERSION");
        http:Request request = new;
        json createEnterpriseCacheJsonPayload ={"location": location, "sku": {"name": skuName, "capacity": skuCapacity}, "zones": ["1", "2", "3"], "properties": {"minimumTlsVersion": "1.2"}, "tags": {"tag1": "value1"}};
    ...
    # + resourceGroupName - Resource Group Name where Redis Enterprise found.
    # + return - If successful, returns EnterpriseClusterInstance. Else returns error. 
    remote function getRedisEnterprise(string redisEnterpriseClusterName, string resourceGroupName) returns @tainted RedisEnterpriseCacheInstance|error {
        ...
        if (response is http:Response) {
            ...
            if(statusCode == "401"){
                ...
                if(payload is json){
                    RedisEnterpriseCacheInstance getRedisEnterpriseCacheResponse = check <@untainted>payload.cloneWithType(RedisEnterpriseCacheInstance);
    ...
    # + resourceGroupName - Resource Group Name where Redis Enterprise found.
    # + return - If successful, returns StatusCode. Else returns error. 
    remote function deleteRedisEnterprise(string redisEnterpriseClusterName, string resourceGroupName) returns @tainted StatusCode|error {
    ...
    remote function listRedisEnterprise() returns @tainted StatusCode|error {
        ...
        var path = "/providers/Microsoft.Cache/redisEnterprise?api-version="+config:getAsString("ENTERPRISE_API_VERSION");
        http:Request request = new;
        var response = self.AzureRedisClient->get(<@untainted>path, request);
        if (response is http:Response) {
            string statusCode = response.statusCode.toString();
    ...
    remote function listRedisEnterpriseByResourceGroup(string resourceGroupName) returns @tainted StatusCode|error {
        ...
        var path = "/resourceGroups/"+resourceGroupName+"/providers/Microsoft.Cache/redisEnterprise?api-version="+config:getAsString("ENTERPRISE_API_VERSION");
        http:Request request = new;
        var response = self.AzureRedisClient->get(<@untainted>path, request);
        if (response is http:Response) {
            string statusCode = response.statusCode.toString();
    ...
    # + resourceGroupName - Resource Group Name where Redis Enterprise found.
    # + return - If successful, returns EnterpriseClusterInstance. Else returns error. 
    remote function updateRedisEnterprise(string redisEnterpriseClusterName, string resourceGroupName) returns @tainted StatusCode|error {
